    ...
    class Program
    {
        ...
        async Task CompileFilesAsync(IEnumerable<string> sassFiles)
        {
            try
            {
                ...
                using var autoprefixer = new Autoprefixer(() => new V8JsEngineFactory().CreateEngine());
                foreach(var file in sassFiles)
                {
                    ...
                    string content = result.CompiledContent;
                    if(Options.Prefix == true)
                    {
                        var autoprefixResult = autoprefixer.Process(result.CompiledContent, options);
                        content = autoprefixResult.ProcessedContent;
                    }
                    var newFile = fileInfo.FullName.Replace(fileInfo.Extension, ".css");

                    if(File.Exists(newFile) && content.ReplaceLineEndings() == (await File.ReadAllTextAsync(newFile)).ReplaceLineEndings())
                        continue;
